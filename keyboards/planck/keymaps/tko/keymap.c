...
#define SH_QUOT MT(MOD_RSFT,SE_QUOT)
...
#ifdef AUDIO_ENABLE
// feedback songs
float confirm_song[][2]         = SONG(COIN_SOUND);
float reject_song[][2]          = SONG(SONIC_RING);

// dynamic macro songs
float dmacro_recs_song[][2]     = SONG(ZELDA_TREASURE);
float dmacro_rece_song[][2]     = SONG(CAPS_LOCK_ON_SOUND);
float dmacro_play_song[][2]     = SONG(TERMINAL_SOUND);

// layer toggle songs
float base_song[][2]            = SONG(SONIC_RING);
float hyper_song[][2]           = SONG(CHROMATIC_SOUND);
float rotary_song[][2]          = SONG(MINOR_SOUND);
float raise1_song[][2]          = SONG(CHROMATIC_SOUND);
float raise2_song[][2]          = SONG(MINOR_SOUND);
float lower1_song[][2]          = SONG(CHROMATIC_SOUND);
float lower2_song[][2]          = SONG(MINOR_SOUND);

// shortcut songs
float caps_on_song[][2]         = SONG(ZELDA_TREASURE);
float caps_off_song[][2]        = SONG(CAPS_LOCK_ON_SOUND);
float save_song[][2]            = SONG(COIN_SOUND);
float cut_song[][2]             = SONG(VIOLIN_SOUND);
float copy_song[][2]            = SONG(VIOLIN_SOUND);
float paste_song[][2]           = SONG(UNICODE_LINUX);
float undo_song[][2]            = SONG(SCROLL_LOCK_ON_SOUND);
float redo_song[][2]            = SONG(ONE_UP_SOUND);
#endif
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    */
    ...
    [_BASE] = LAYOUT_planck_grid(
        ...
        CTL_TAB, KC_A,    KC_S,    KC_D,    KC_F,   KC_G,     KC_H,     KC_J,   KC_K,    KC_L,    SE_SCLN, KC_ENT,
        SH_ESC,  KC_Z,    KC_X,    KC_C,    KC_V,   KC_B,     KC_N,     KC_M,   SE_COMM, SE_DOT,  SE_SLSH, SH_QUOT,
    ...
    */
    [_LOWER1] = LAYOUT_planck_grid(
        _______, KC_1,    KC_2,    KC_3,    SE_DLR, SE_ASTR, SE_SLSH, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, _______,
        _______, KC_4,    KC_5,    KC_6,    SE_DOT, SE_PLUS, SE_EQL,  SE_LBRC, SE_RBRC, SE_LCBR, SE_RCBR, _______,
        _______, KC_7,    KC_8,    KC_9,    KC_0,   SE_MINS, SE_UNDS, SE_LPRN, SE_RPRN, SE_LABK, SE_RABK, _______,
    ...
    */
    [_RAISE1] = LAYOUT_planck_grid(
        _______, KC_EXLM, KC_AT,   KC_HASH, XXXXXXX, XXXXXXX, XXXXXXX, KC_WH_L, KC_WH_D, KC_WH_U, KC_WH_R,  SE_ARNG,
        _______, SE_DLR,  SE_PERC, SE_CIRC, XXXXXXX, XXXXXXX, KC_LEFT, KC_DOWN, KC_UP,  KC_RIGHT, SE_ODIA,  SE_ADIA,
        _______, SE_AMPR, SE_ASTR, SE_LPRN, SE_RPRN, XXXXXXX, XXXXXXX, SE_TILD, SE_GRV,  SE_PIPE, SE_BSLS,  _______,
    ...
    */
    [_RAISE2] = LAYOUT_planck_grid(
        ...
        _______, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, KC_BTN1, KC_BTN2, KC_ACL1, KC_ACL2, XXXXXXX, _______,
